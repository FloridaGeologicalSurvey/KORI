# -*- coding: utf-8 -*-
"""
Created on Mon April 11, 2014

Abstract:
This is a module for handling falmouth data, revision 2.
...
############## Functions for Handling Raw Falmouth Files ######################
def parse(falmouthFile, returnType):
    ...
    Allowed returnTypes: 'data', 'header', "data header"
    Returns a list of header information, the data table, or the header for the data table from the file.
...
def parseTimestamps(data):
    """
    Parses a falmouth data table, returns the max and minimum timestamps as a 
...
def parseDates(data):
    ...
    Scans the data table generated by parseFalmouthFile('somefile', 'data') 
        and returns the min and max date as a list of datetime.date objects
    """
    dates = []
    for i in data:
...
def cast(data):
    """
    Casts Falmouth Data into proper types for SQL
    """
    castData = []
    for i in data:
        #required to filter bad data lines occasionally generated by the sensors
...
def pandizeDB(dbData):
    ...
    pdData.columns = ['avn', 'ave', 'aspd', 'avdir', 'atlt', 'date_time', 'cond', 'temp', 'pres', 'hdng', 'batt', 'vx', 'vy', 'tx', 'ty', 'hx', 'hy', 'hz', 'vn', 've', 'stemp', 'sv1', 'vab', 'vcd', 'vef', 'vgh', 'id','dev_deply_ky', "salt", "depth", "sv2"]
    pdData = pdData.sort_index()
    return pdData

def pandizeCompare(df1, df2):
    """Compares two Panda Dataframes and asserts equality"""
    try: 
...
def writeFalmouthDataTypeA(falmouthFile, deploy_key, connection_info):
    """ 
    Falmouth files come in two types, differentiated by the column headings on the data table
    This deals with "type A" 
    Use the famouthClassify function to descriminate between the two
    """
    ...
    sqlQuery = cur.mogrify("SELECT * FROM dev_log WHERE dev_deply_ky = %s AND (date_time BETWEEN %s AND %s);", (deploy_key, startEnd[0], startEnd[1]))
    ...
    if len(rows)  == 0:
        ...
        try:        
            ...
            return
        for i in castBlock:
            cur.execute("""INSERT INTO dev_log (avn, ave, aspd, avdir, atlt, date_time, cond, temp, pres, hdng, batt, vx, vy, tx, ty, hx, hy, hz, vn, ve, stemp, sv1, vab, vcd, vef, vgh, dev_deply_ky)
...
def writeFalmouthDataTypeB(falmouthFile, deploy_key, connection_info):
    ...
    if len(rows)  == 0:
        ...
        try:        
            ...
            return
        for i in castBlock:
            cur.execute("""INSERT INTO dev_log (avn, ave, aspd, avdir, atlt, date_time, cond, temp, pres, salt, sv2, hdng, batt, vx, vy, tx, ty, hx, hy, hz, vn, ve, stemp, sv1, vab, vcd, vef, vgh, dev_deply_ky)
...
def retrieveRange(startEnd, deploy_key, connection_info):   
    """
    Querys the SQL dev_log table for any records for a given deploy key between a date time range
    """    
    con = psycopg2.connect(dsn=None, 
        host= connection_info[0],
        database= connection_info[1],
	...
	#establish dictionary database cursor to read DB
    cur = con.cursor()
    sqlQuery = sqlQuery = cur.mogrify("SELECT * FROM dev_log WHERE dev_deply_ky = %s AND (date_time BETWEEN %s AND %s);", (deploy_key, startEnd[0], startEnd[1]))
	#set SQL query to run against DB tables
    cur.execute(sqlQuery)
...
"""
These functions are depricated and no longer used. They are included in this 
file only for the sake of running old code that rely on them.
"""
...
def parseDeployTable(deployTable):
	...
	for i in deployTable:
		...
		if i[4] is None:
			...
			newEnd = i[4]
		newDeployTable.append( [ i[2] , newStart , newEnd, i[10] ] )
	return newDeployTable

def retrieveColumnInfo(sqlQuery, connection_info):
    con = psycopg2.connect(dsn=None, 
        host= connection_info[0],
        database= connection_info[1],
        user= connection_info[2],
